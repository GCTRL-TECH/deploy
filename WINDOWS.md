# Ground Control on Windows

This guide walks through a complete installation of Ground Control (GCTRL) on Windows. GCTRL runs as Docker containers; on Windows those containers run through **Docker Desktop with the WSL 2 backend**. You run the installer from inside a WSL 2 (Ubuntu) terminal, and you access the dashboard from your normal Windows browser.

For best performance you will also install **native Ollama for Windows** (GPU-accelerated) and point GCTRL at it. This is covered in step 3 and step 6.
...
This step is what unlocks GPU acceleration. The Ollama bundled inside Docker is **CPU-only** — Docker cannot reach your NVIDIA GPU. Installing Ollama natively on Windows lets GCTRL use the GPU for both RAG/agents and KEX extraction and embeddings.
...
You will connect GCTRL to this native Ollama in step 6.
...
Without this switch, GCTRL keeps using the bundled CPU-only Ollama — fine to start, slow for real work.
